...
function NeonLib:CreateWindow(options)
	...
	searchFrame.Position        = UDim2.new(1, -135, 0.5, -13)
	...
	local function toggleSearch()
		...
		if searchOpen then
			tween(searchFrame, {Size = UDim2.new(0, 130, 0, 26)}, 0.25, Enum.EasingStyle.Back)
	...
	bodyFrame.Name             = "Body"
	bodyFrame.Size             = UDim2.new(1, 0, 1, -94)
	...
	function Window:AddTab(tabOptions)
		...
		function Tab:AddTextBox(tbOptions)
			...
			box.Parent              = row
			addCorner(box, 3)
			addStroke(box, Theme.BorderDim, 1, 0.4)
			addPadding(box, 0, 0, 8, 8)

			box.Focused:Connect(function()
				addStroke(box, Theme.Accent, 1, 0.2)
			end)
